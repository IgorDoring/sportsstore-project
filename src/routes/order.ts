...
export const createOrderRoutes = (app: Express) => {
    ...
    app.get(
        '/signin-google',
        passport.authenticate('google', { successRedirect: '/checkout', keepSessionInfo: true })
    )
        ...
        if (!req.session.orderData && req.user) {
            req.session.orderData = {
                ...
                address: await AddressValidator.validate(
                    (await customer_repository.getCustomerAddress(req.user?.id ?? 0)) ?? {}
                )
            }
        }
    ...
    app.get('/checkout/:id', (req, resp) => {
        resp.render('order_complete', { id: req.params.id, pageSize: req.session.pageSize ?? 3 })
    })
}
